...
function ChatService(props) {
    ...
    useEffect(() => {
            async function loadData() {
                ...
                setShopAll(res.data);
                const socketConnectS = new WebSocket(`ws://localhost:8202/mellow-ecommerce-floor/chat/${customer.id}_${res?.data[0]?.shopId}`);
    ...
    function activeTicker(id, name, img) {
        ...
        setHeaderImg(img);
        setHeaderName(name);
        const socketConnect = new WebSocket(`ws://localhost:8202/mellow-ecommerce-floor/chat/${customer.id}_${id}`);
