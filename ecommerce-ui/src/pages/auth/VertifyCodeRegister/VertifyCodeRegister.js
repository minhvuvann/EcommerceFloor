...
import styles from "./VertifyCodeRegister.module.scss";
...
function VerifyCodeRegister(props) {
    ...
    async function submitCode() {
        ...
        if (codeSession != null && code === atob(codeSession)) {
            setConfirmed(true);
            const result = await activeUser(id, "Vũ Văn Minh");
            localStorage.clear();
            localStorage.setItem("cs-id", id);
            console.log(result);
            localStorage.setItem("cs-obj", JSON.stringify(result?.data));
            setTimeout(() => {
                setConfirmed(false);
            }, 5000);
        } else {
                        ...
                        <form className={cx("form-vertify")} onSubmit={handleSubmit}>
                            <FormControl className={cx("input")}>
                                <div className={cx("form-header")}>
                                    <a href={config.routes.register} style={{color: "#ff5588", marginRight: "6rem"}}>
